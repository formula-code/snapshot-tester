...
class BenchmarkDiscovery:
    ...
    def _discover_class_benchmarks(self, class_node: ast.ClassDef, module_path: str) -> List[BenchmarkInfo]:
            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        if target.id == 'params' and (isinstance(node.value, ast.List) or isinstance(node.value, ast.Tuple)):
