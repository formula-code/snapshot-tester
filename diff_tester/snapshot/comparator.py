...
class Comparator:
    ...
    def compare(self, actual: Any, expected: Any) -> ComparisonResult:
        ...
        try:
            ...
            return ComparisonResult(
                match=False,
                error_message="No comparison strategy could handle these types"
            )
            
        except Exception as e:
            return ComparisonResult(
                match=False,
                error_message=f"Comparison failed with exception: {e}"
            )
    ...
    def _compare_class_instance(self, actual: Any, expected: Dict[str, Any]) -> ComparisonResult:
        ...
        if not hasattr(actual, '__class__'):
            return ComparisonResult(
                match=False,
                error_message=f"Expected class instance but got {type(actual)}"
            )
        ...
        for key, expected_value in expected_dict.items():
            if key not in actual_dict:
                return ComparisonResult(
                    match=False,
                    error_message=f"Missing attribute '{key}' in actual instance"
                )
        ...
        for key in actual_dict:
            if key not in expected_dict:
                return ComparisonResult(
                    match=False,
                    error_message=f"Extra attribute '{key}' in actual instance"
                )
    ...
    def _compare_numpy_arrays(self, actual: Any, expected: Any) -> Optional[ComparisonResult]:
        ...
        try:
            ...
            return ComparisonResult(
                match=False,
                error_message=f"numpy.allclose failed: {e}"
            )
    ...
    def _compare_scalars(self, actual: Any, expected: Any) -> Optional[ComparisonResult]:
        ...
        try:
            match = np.isclose(actual_np, expected_np,
                             rtol=self.config.rtol,
                             atol=self.config.atol,
                             equal_nan=self.config.equal_nan)
            ...
            return ComparisonResult(
                match=False,
                error_message=f"Scalar comparison failed: {e}"
            )
    ...
    def _compare_dicts(self, actual: Any, expected: Any) -> Optional[ComparisonResult]:
        ...
        if actual_keys != expected_keys:
            ...
            return ComparisonResult(
                match=False,
                error_message=f"Dict keys differ: missing={missing}, extra={extra}"
            )
    ...
    def _compare_fallback(self, actual: Any, expected: Any) -> Optional[ComparisonResult]:
        ...
        try:
            ...
            return ComparisonResult(
                match=False,
                error_message=f"Fallback comparison failed: {e}"
            )
    ...
    def compare_multiple(self, 
                        ...
                        expected_values: List[Any]) -> List[ComparisonResult]:
