...
class TestBasicStorage:
    ...
    def test_store_simple_snapshot(self, manager):
        ...
        sidecar_path = manager.store_snapshot(
        ...
        # store_snapshot returns the JSON metadata sidecar path; the payload
    ...
    def test_load_simple_snapshot(self, manager):
        ...
        loaded = manager.load_snapshot(
            benchmark_name="test_bench", module_path="test_module", parameters=(1, 2, 3)
        )
    ...
    def test_store_and_load_numpy_array(self, manager):
        ...
        loaded_value, _ = manager.load_snapshot(
            benchmark_name="array_bench", module_path="test_module", parameters=()
        )
    ...
    def test_metadata_stored(self, manager):
        ...
        _, metadata = manager.load_snapshot(
            benchmark_name="meta_bench", module_path="test_module", parameters=(1,)
        )
...
class TestParameterHandling:
    ...
    def test_different_parameters_different_snapshots(self, manager):
        ...
        loaded1, _ = manager.load_snapshot(
            benchmark_name="param_bench", module_path="test_module", parameters=(1, 2)
        )

        loaded2, _ = manager.load_snapshot(
            benchmark_name="param_bench", module_path="test_module", parameters=(3, 4)
        )
    ...
    def test_no_parameters(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="no_param_bench", module_path="test_module", parameters=()
        )
    ...
    def test_complex_parameter_types(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="complex_params", module_path="test_module", parameters=params
        )
...
class TestUnpicklableObjects:
    ...
    def test_generator_serialization(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="gen_bench", module_path="test_module", parameters=()
        )
    ...
    def test_lambda_serialization(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="lambda_bench", module_path="test_module", parameters=()
        )
    ...
    def test_class_instance_serialization(self, manager):
        ...
        try:
            ...
            loaded, _ = manager.load_snapshot(
                benchmark_name="class_bench", module_path="test_module", parameters=()
            )
    ...
    def test_nested_unpicklable(self, manager):
        ...
        data = {"number": 42, "generator": gen(), "list": [1, 2, 3]}
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="nested_unpickle", module_path="test_module", parameters=()
        )
...
class TestFailedCaptures:
    ...
    def test_store_failed_capture(self, manager):
        ...
        is_failed = manager.is_failed_capture(
            benchmark_name="failed_bench", module_path="test_module", parameters=(1, 2)
        )
    ...
    def test_is_failed_capture_false_for_success(self, manager):
        ...
        is_failed = manager.is_failed_capture(
            benchmark_name="success_bench", module_path="test_module", parameters=()
        )
    ...
    def test_load_failed_capture_returns_none(self, manager):
        ...
        loaded = manager.load_snapshot(
            benchmark_name="failed_bench", module_path="test_module", parameters=()
        )
...
class TestSnapshotPaths:
    ...
    def test_snapshot_path_structure(self, manager, temp_snapshot_dir):
        ...
        # Payload lives in <snapshot_dir>/snapshots.db; the per-test JSON
        # sidecar still lives at <snapshot_dir>/<module>/<benchmark>/<hash>.json
        # because downstream tooling consumes it.
        assert (temp_snapshot_dir / "snapshots.db").exists()
        ...
        # No pickle files anywhere — everything in SQLite.
        assert list(temp_snapshot_dir.rglob("*.pkl")) == []
        assert list(temp_snapshot_dir.rglob("*.pkl.gz")) == []
...
class TestEdgeCases:
    ...
    def test_very_large_snapshot(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="large_bench", module_path="test_module", parameters=()
        )

        assert np.array_equal(loaded, large_array)

    def test_payload_is_compressed_in_db(self, temp_snapshot_dir):
        """Every payload is gzipped in the SQLite ``blobs`` table — there is no
        size threshold under the SQLite backend."""
        import sqlite3

        manager = SnapshotManager(temp_snapshot_dir)
        data = {"message": "compressed" * 1000}  # large enough to benefit from gzip
        ...
        # Compressed bytes are strictly smaller than raw bytes for repetitive data.
        with sqlite3.connect(str(manager.db_path)) as conn:
            row = conn.execute("SELECT compressed_size, raw_size FROM blobs").fetchone()
        compressed, raw = row
        assert compressed > 0 and raw > 0
    ...
    def test_identical_payloads_are_deduplicated(self, temp_snapshot_dir):
        """Two benchmarks producing identical return values share a single blob."""
        import sqlite3

        manager = SnapshotManager(temp_snapshot_dir)
        same_value = [1, 2, 3, 4, 5]
        ...
        with sqlite3.connect(str(manager.db_path)) as conn:
            ...
            refcount = conn.execute("SELECT refcount FROM blobs").fetchone()[0]
    ...
    def test_special_characters_in_names(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="test-bench.v2", module_path="my.module", parameters=()
        )
    ...
    def test_nested_directory_creation(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="bench", module_path="deeply/nested/module/path", parameters=()
        )
    ...
    def test_overwrite_existing_snapshot(self, manager):
        ...
        loaded, _ = manager.load_snapshot(
            benchmark_name="overwrite_test", module_path="test_module", parameters=params
        )
    ...
    def test_load_nonexistent_snapshot(self, manager):
        ...
        loaded = manager.load_snapshot(
            benchmark_name="nonexistent", module_path="test_module", parameters=()
        )
...
class TestGitIntegration:
    ...
    def test_git_commit_in_metadata(self, manager):
        ...
        _, metadata = manager.load_snapshot(
            benchmark_name="git_test", module_path="test_module", parameters=()
        )
    ...
    def test_platform_info_in_metadata(self, manager):
        ...
        _, metadata = manager.load_snapshot(
            benchmark_name="platform_test", module_path="test_module", parameters=()
        )
...
class TestConcurrency:
    ...
    def test_multiple_snapshots_same_benchmark(self, manager):
        ...
        for i in range(10):
            loaded, _ = manager.load_snapshot(
                benchmark_name="concurrent_bench", module_path="test_module", parameters=(i,)
            )
            assert loaded == i * 2
